...
CLAIM_IDENTITY_TRUST_AGE_SECONDS = _env_int(
    "NIMHUNT_CLAIM_IDENTITY_TRUST_AGE_SECONDS", 30 * 24 * 60 * 60
)
CLAIM_SIGNER_HISTORY_NEGATIVE_CACHE_SECONDS = _env_int(
    "NIMHUNT_CLAIM_SIGNER_HISTORY_NEGATIVE_CACHE_SECONDS", 24 * 60 * 60
)
CLAIM_SIGNER_HISTORY_FAILURE_RETRY_SECONDS = _env_int(
    "NIMHUNT_CLAIM_SIGNER_HISTORY_FAILURE_RETRY_SECONDS", 10 * 60
)
CLAIM_SIGNER_HISTORY_PAGE_SIZE = _env_int(
    "NIMHUNT_CLAIM_SIGNER_HISTORY_PAGE_SIZE", 500
)
CLAIM_SIGNER_HISTORY_MAX_PAGES = _env_int(
    "NIMHUNT_CLAIM_SIGNER_HISTORY_MAX_PAGES", 100
)
# Funding-origin observation is deliberately much smaller than the signer-age
# walk.  A complete bounded history is required before an origin is recorded.
CLAIM_FUNDING_HISTORY_PAGE_SIZE = _env_int(
    "NIMHUNT_CLAIM_FUNDING_HISTORY_PAGE_SIZE", 100
)
CLAIM_FUNDING_HISTORY_MAX_PAGES = _env_int(
    "NIMHUNT_CLAIM_FUNDING_HISTORY_MAX_PAGES", 4
)
CLAIM_FUNDING_SOURCE_SAMPLE_SIZE = _env_int(
    "NIMHUNT_CLAIM_FUNDING_SOURCE_SAMPLE_SIZE", 100
)
CLAIM_FUNDING_SOURCE_SERVICE_DEGREE = _env_int(
    "NIMHUNT_CLAIM_FUNDING_SOURCE_SERVICE_DEGREE", 50
)
CLAIM_FUNDING_CLUSTER_MIN_CLAIMANTS = _env_int(
    "NIMHUNT_CLAIM_FUNDING_CLUSTER_MIN_CLAIMANTS", 5
)
CLAIM_FUNDING_CLUSTER_WINDOW_SECONDS = _env_int(
    "NIMHUNT_CLAIM_FUNDING_CLUSTER_WINDOW_SECONDS", 7 * 24 * 60 * 60
)
CLAIM_FUNDING_CLUSTER_MEMBER_LIMIT = _env_int(
    "NIMHUNT_CLAIM_FUNDING_CLUSTER_MEMBER_LIMIT", 16
)
CLAIM_FUNDING_CACHE_SECONDS = _env_int(
    "NIMHUNT_CLAIM_FUNDING_CACHE_SECONDS", 30 * 24 * 60 * 60
)
CLAIM_FIRST_LOCATION_MISMATCH_METRES = _env_int(
    "NIMHUNT_CLAIM_FIRST_LOCATION_MISMATCH_METRES", 1_500_000
)
CLAIM_FIRST_LOCATION_COOLDOWN_SECONDS = _env_int(
    "NIMHUNT_CLAIM_FIRST_LOCATION_COOLDOWN_SECONDS", 24 * 60 * 60
)
CLAIM_FIRST_LOCATION_SECOND_COOLDOWN_SECONDS = _env_int(
    "NIMHUNT_CLAIM_FIRST_LOCATION_SECOND_COOLDOWN_SECONDS", 7 * 24 * 60 * 60
)
CLAIM_FIRST_LOCATION_UNKNOWN_RETRY_SECONDS = _env_int(
    "NIMHUNT_CLAIM_FIRST_LOCATION_UNKNOWN_RETRY_SECONDS", 15 * 60
)
IP_GEOLOCATION_TIMEOUT_SECONDS = _env_int("NIMHUNT_IP_GEOLOCATION_TIMEOUT_SECONDS", 4)
