...
METADATA_NEXT_SPOT_DEPOSIT_KEY_INDEX = "next_spot_deposit_key_index"
...
# An admission row is created before potentially expensive deposit derivation.
# Pending rows may be reclaimed, but their allocated key indexes remain burned
# in the monotonic app_metadata counter.
DRAFT_CREATION_ADMISSION_TABLE_NAME = "draft_creation_admission"
DRAFT_CREATION_ADMISSION_ID = "id"
DRAFT_CREATION_ADMISSION_USER_ID = "user_id"
DRAFT_CREATION_ADMISSION_KEY_INDEX = "deposit_key_index"
DRAFT_CREATION_ADMISSION_STATUS = "status"
DRAFT_CREATION_ADMISSION_CREATED_AT = "created_at"
DRAFT_CREATION_ADMISSION_CONSUMED_AT = "consumed_at"
DRAFT_ADMISSION_PENDING = "pending"
DRAFT_ADMISSION_CONSUMED = "consumed"
CREATE_DRAFT_CREATION_ADMISSION_TABLE = f"""
CREATE TABLE IF NOT EXISTS {DRAFT_CREATION_ADMISSION_TABLE_NAME} (
    {DRAFT_CREATION_ADMISSION_ID} INTEGER PRIMARY KEY AUTOINCREMENT,
    {DRAFT_CREATION_ADMISSION_USER_ID} INTEGER NOT NULL,
    {DRAFT_CREATION_ADMISSION_KEY_INDEX} INTEGER NOT NULL UNIQUE,
    {DRAFT_CREATION_ADMISSION_STATUS} TEXT NOT NULL
        CHECK ({DRAFT_CREATION_ADMISSION_STATUS} IN ('{DRAFT_ADMISSION_PENDING}', '{DRAFT_ADMISSION_CONSUMED}')),
    {DRAFT_CREATION_ADMISSION_CREATED_AT} INTEGER NOT NULL,
    {DRAFT_CREATION_ADMISSION_CONSUMED_AT} INTEGER,
    FOREIGN KEY ({DRAFT_CREATION_ADMISSION_USER_ID}) REFERENCES {USER_TABLE_NAME}({USER_ID}) ON DELETE RESTRICT
);
CREATE INDEX IF NOT EXISTS idx_draft_creation_admission_user_time
ON {DRAFT_CREATION_ADMISSION_TABLE_NAME}({DRAFT_CREATION_ADMISSION_USER_ID}, {DRAFT_CREATION_ADMISSION_CREATED_AT});
"""
...
async def init_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.executescript(CREATE_DRAFT_CREATION_ADMISSION_TABLE)
