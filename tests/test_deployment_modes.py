...
class PublicDeploymentValidationTest(unittest.TestCase):
    ...
    def test_public_signer_probe_derives_without_logging_secrets(self):
        derived = mock.Mock(address=VALID_FEE_ADDRESS)
        with (
            mock.patch.object(const, "PUBLIC_DEPLOYMENT", True),
            mock.patch.object(
                main.wallet, "derive_spot_deposit_address", return_value=derived
            ) as derive,
        ):
