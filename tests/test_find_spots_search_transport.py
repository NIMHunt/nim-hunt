...
CACHE_VERSION = "wrapped-search-v1-20260803"
...
def test_find_spots_loads_the_search_transport_before_the_page_module():
    ...
    assert (
        f"/static/find_spots_bootstrap.js?v={CACHE_VERSION}"
        in template
    )
    assert 'src="/static/find_spots.js?' not in template
    assert (
        f"./find_spots_search_transport.js?v={CACHE_VERSION}"
        in bootstrap
    )
    assert "installFindSpotsSearchTransport();" in bootstrap
    assert f"./find_spots.js?v={CACHE_VERSION}" in bootstrap
...
def test_search_transport_is_scoped_to_visible_spot_searches():
    ...
    assert "if (url.pathname !== SPOT_SEARCH_PATH) return [];" in transport
