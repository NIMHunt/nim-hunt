...
SOURCE_BURST_LIMIT = int(os.getenv("NIMHUNT_USER_REGISTRATION_SOURCE_BURST_LIMIT", "40"))
...
async def get_or_create_public_user(
    ...
    burst = [stamp for stamp in source_stamps if stamp > int(now) - SOURCE_BURST_WINDOW_SECONDS]
    if len(burst) >= max(1, SOURCE_BURST_LIMIT):
        raise RegistrationRateLimited(
            retry_at=min(burst) + SOURCE_BURST_WINDOW_SECONDS + 1
        )
